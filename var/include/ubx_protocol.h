/* Generated by gen_ubx from /home/djim/Documents/paparazzi/conf/ubx.xml */
/* Please DO NOT EDIT */
...
#define UbxSend_CFG_PRT_POLL() { \
  UbxHeader(UBX_CFG_ID, UBX_CFG_PRT_POLL_ID, 0);\
  UbxTrailer();\
}
...
#define UbxSend_CFG_MSG(class,msgid,rate) { \
  UbxHeader(UBX_CFG_ID, UBX_CFG_MSG_ID, 3);\
  uint8_t _class = class; UbxSend1ByAddr((uint8_t*)&_class);\
  uint8_t _msgid = msgid; UbxSend1ByAddr((uint8_t*)&_msgid);\
  uint8_t _rate = rate; UbxSend1ByAddr((uint8_t*)&_rate);\
  UbxTrailer();\
}
...
#define UbxSend_RXM_RAW(itow,week,numsv,reserverd1,cpmes,prmes,domes,sv,mesqi,cno,lli) { \
  UbxHeader(UBX_RXM_ID, UBX_RXM_RAW_ID, 32);\
  int32_t _itow = itow; UbxSend4ByAddr((uint8_t*)&_itow);\
  int16_t _week = week; UbxSend2ByAddr((uint8_t*)&_week);\
  uint8_t _numsv = numsv; UbxSend1ByAddr((uint8_t*)&_numsv);\
  uint8_t _reserverd1 = reserverd1; UbxSend1ByAddr((uint8_t*)&_reserverd1);\
  double _cpmes = cpmes; UbxSend8ByAddr((uint8_t*)&_cpmes);\
  double _prmes = prmes; UbxSend8ByAddr((uint8_t*)&_prmes);\
  float _domes = domes; UbxSend4ByAddr((uint8_t*)&_domes);\
  uint8_t _sv = sv; UbxSend1ByAddr((uint8_t*)&_sv);\
  int8_t _mesqi = mesqi; UbxSend1ByAddr((uint8_t*)&_mesqi);\
  int8_t _cno = cno; UbxSend1ByAddr((uint8_t*)&_cno);\
  uint8_t _lli = lli; UbxSend1ByAddr((uint8_t*)&_lli);\
  UbxTrailer();\
}
...
#define UbxSend_RXM_SFRB(chn,svid,dwrd0,dwrd1,dwrd2,dwrd3,dwrd4,dwrd5,dwrd6,dwrd7,dwrd8,dwrd9) { \
  UbxHeader(UBX_RXM_ID, UBX_RXM_SFRB_ID, 42);\
  uint8_t _chn = chn; UbxSend1ByAddr((uint8_t*)&_chn);\
  uint8_t _svid = svid; UbxSend1ByAddr((uint8_t*)&_svid);\
  uint32_t _dwrd0 = dwrd0; UbxSend4ByAddr((uint8_t*)&_dwrd0);\
  uint32_t _dwrd1 = dwrd1; UbxSend4ByAddr((uint8_t*)&_dwrd1);\
  uint32_t _dwrd2 = dwrd2; UbxSend4ByAddr((uint8_t*)&_dwrd2);\
  uint32_t _dwrd3 = dwrd3; UbxSend4ByAddr((uint8_t*)&_dwrd3);\
  uint32_t _dwrd4 = dwrd4; UbxSend4ByAddr((uint8_t*)&_dwrd4);\
  uint32_t _dwrd5 = dwrd5; UbxSend4ByAddr((uint8_t*)&_dwrd5);\
  uint32_t _dwrd6 = dwrd6; UbxSend4ByAddr((uint8_t*)&_dwrd6);\
  uint32_t _dwrd7 = dwrd7; UbxSend4ByAddr((uint8_t*)&_dwrd7);\
  uint32_t _dwrd8 = dwrd8; UbxSend4ByAddr((uint8_t*)&_dwrd8);\
  uint32_t _dwrd9 = dwrd9; UbxSend4ByAddr((uint8_t*)&_dwrd9);\
  UbxTrailer();\
}
